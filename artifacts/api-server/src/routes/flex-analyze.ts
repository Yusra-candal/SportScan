...
function proxyToFlexAnalyzer(req: Request, res: Response) {
  const options: http.RequestOptions = {
    hostname: "localhost",
    port: 5000,
    path: "/flex-api/analyze",
    method: "POST",
    headers: req.headers,
  };
  ...
  proxyReq.on("error", (err: Error) => {
    req.log.error({ err }, "flex-analyze proxy error");
    if (!res.headersSent) {
...
router.post("/analyze", proxyToFlexAnalyzer);
