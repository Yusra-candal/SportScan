...
function proxyToRunAnalyzer(req: Request, res: Response) {
  const options: http.RequestOptions = {
    hostname: "localhost",
    port: 5000,
    path: "/run-api/analyze",
    method: "POST",
    headers: req.headers,
  };
  ...
  proxyReq.on("error", (err: Error) => {
    req.log.error({ err }, "run-analyze proxy error");
    if (!res.headersSent) {
...
router.post("/analyze", proxyToRunAnalyzer);
